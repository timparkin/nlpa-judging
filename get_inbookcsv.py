...
project_extras = [(1,649),(1,3511),(1,70),(1,3908),]
...
if __name__ == "__main__":
    ...
    inbook_csv = yaml_args['inbook_csv']
...
# Loop on the adjustment items
    for judge_id, score in adjust_set.items():
        ...
        for i in range(len(score)):
            ...
            if (r['File,name'] == '00_project_description.jpg'):
                ...
                #organiser_entry_adjust_label_lookup[ (str(user_id), str(entry_id), judge_id) ] = entry_label
                #organiser_entry_score_lookup[(str(user_id), str(entry_id), judge_id)] += entry_adjust
    ...
    for i in range(len(df)):
        ...
        try:
            ...
            print('value error in nlpaentries read')
            continue


        gender_mapped = gender_mapper.get(d.get_gender(row['name'].split(' ')[0]),'unknown')

        if gender_mapped in gender_count:
            gender_count[ gender_mapped ] += 1
        else:
            gender_count[ gender_mapped ] = 1
        ...
        if not isinstance(url,str):
            print('url not string in nlpaentries read')
            continue
        name = row['name']
    ...
    for i in range(len(df)):
        ...
        if '__' in filename:
            id, entry_id = [int(x) for x in filename[:-4].rsplit('__', maxsplit=2)[1:]]
        else:
            id, entry_id = [int(x) for x in filename[:-4].rsplit('_', maxsplit=2)[1:]]
        if int(entry_id) not in inbook:
            inbook.append(int(entry_id))

    log_ids = []
    inbook_count = 0
    country_count = {}
    with open('book_entries.csv', mode='w') as p:
        ...
        for c, w in cat_winners.items():
            for id, prize in w.items():
                ...
                if int(id) not in inbook:
                    continue
                inbook.remove(int(id))
                inbook_count += 1
                ...
                if not isinstance(country, str):
                    if e['id'] in country_lookup:
                        country = country_lookup[e['id']]
                    else:
                        print(e.id)
                        sys.exit()
                ...
                gender_mapped = gender_mapper.get(d.get_gender(e['name'].split(' ')[0]), 'unknown')

                if id not in log_ids:
                    if gender_mapped in gender_count_inbook:
                        gender_count_inbook[gender_mapped] += 1
                    else:
                        gender_count_inbook[gender_mapped] = 1
                log_ids.append(id)
                writer.writerow([
        ...
        for id, data in poty_winners.items():
            ...
            for entry_id in entries:
                ...
                if int(entry_id) not in inbook:
                    continue
                inbook.remove(int(entry_id))
                inbook_count += 1
                directory = entry['entry_category']
                newname = 'poty_%s_%s_%s' % (prize.split(',')[1], e['name'], id)
                print(' -- poty: %s = %s [%s] (%s)' % (prize.split(',')[1], e['name'], entry_id, country))
                gender_mapped = gender_mapper.get(d.get_gender(e['name'].split(' ')[0]), 'unknown')
        ...
        for id, data in project_winners.items():
            ...
            for entry_id in entries:
                ...
                print('%s: %s = %s [%s,%s] (%s)' % (c, prize, e['name'], int(entry_id), id, country))
                gender_mapped = gender_mapper.get(d.get_gender(e['name'].split(' ')[0]), 'unknown')
        ...
        print('len(inbook) at end', len(inbook))
        ...
        for id in inbook:
            ...
            print('%s: %s = %s [%s,%s] (%s)' % (category_lookup[e['entry_category']][3:], prize, e['name'], int(e['id']), id, country))
            gender_mapped = gender_mapper.get(d.get_gender(e['name'].split(' ')[0]), 'unknown')
            ...
            log_ids.append(id)
            writer.writerow([
                category_lookup[e['entry_category']][3:],
                int(e['id']),
                int(e['entry_id']),
                e['entry_filename'],
                e['entry_url'],
    ...
    sorted_countries = dict(sorted(country_count.items(), key=lambda item: item[1], reverse=True))
    for k,v in sorted_countries.items():
        print('%s = %s'%(v,k))



    for id,name in no_country:
        print("{}: '{}',".format(int(id),name))

    total = gender_count['male'] + gender_count['female'] + gender_count['unknown']
    male = round( gender_count['male']*100/total, 2)
    female = round( gender_count['female']*100/total, 2)
    unknown = round( gender_count['unknown']*100/total, 2)
    print('\nIn Competition')
    print('male: {}%, female: {}%, unkown: {}%'.format(male, female, unknown))

    total = gender_count_inbook['male'] + gender_count_inbook['female'] + gender_count_inbook['unknown']
    male = round( gender_count_inbook['male']*100/total, 2)
    female = round( gender_count_inbook['female']*100/total, 2)
    unknown = round( gender_count_inbook['unknown']*100/total, 2)
    print('\nIn Book')
    print('male: {}%, female: {}%, unkown: {}%'.format(male, female, unknown))
